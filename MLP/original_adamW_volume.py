"""
MLP volume analysis on MNIST with AdamW (no L2 weight decay).

Baseline experiment: trains a multi-layer perceptron and records per-epoch metrics:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train an MLP on MNIST and log volume / PII metrics (AdamW baseline)."
    )
...
def build_experiment_name(args: argparse.Namespace) -> str:
    return (
        f"original_{args.init_method}_AdamW"
