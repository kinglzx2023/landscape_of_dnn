...
name = 'original_'+'adamW_'+str(Task)+\
...
for name, param in model.named_parameters():
        for i in range(0,12,2):
            ...
            if name == f'bert.encoder.layer.{i}.attention.self.value.weight':
                ...
                file1.writelines( f"{parts[3]}_{parts[6]}"+','+str(cos_similarity(name))+',')
                volume(param.cpu().data) 
            if name == f'bert.encoder.layer.{i}.attention.output.dense.weight':
                parts = name.split('.')
                file1.writelines( f"{parts[3]}_{parts[5]}"+','+str(cos_similarity(name))+',')
                volume(param.cpu().data) 
            if name == f'bert.encoder.layer.{i}.intermediate.dense.weight':
                parts = name.split('.')
                file1.writelines( f"{parts[3]}_{parts[4]}"+','+str(cos_similarity(name))+',')
                volume(param.cpu().data) 
            if name == f'bert.encoder.layer.{i}.output.dense.weight':
                parts = name.split('.')
                file1.writelines( f"{parts[3]}_{parts[4]}"+','+str(cos_similarity(name))+',')
                volume(param.cpu().data) 
